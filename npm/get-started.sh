...
# Get the version from package.json using absolute path
VERSION=$(node -p "require('$(dirname $(realpath "$0"))/package.json').version")
...
if ! "$BINARY_PATH" --lang typescript 2>&1; then
